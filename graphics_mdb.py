...
_RES_X             = np.clip(int(os.getenv('RES_X')), 640, 15360)
_RES_Y             = np.clip(int(os.getenv('RES_Y')), 480, 8640)
        ...
        for Z in range(CHK.shape[0]):
            for X in range(CHK.shape[1]):
                for Y in range(CHK.shape[2]):
                    FRC_COL_CHK_AVG += CHK[Z, X, Y]
        
        FRC_COL_CHK_AVG /= (_SIZ ** 3)
...
def main():
    ...
    while True:
        ...
        #pygame.event.pump() # (TEST THIS IF NEEDED ?) aimed at forcing events to continue even when window is not focused
